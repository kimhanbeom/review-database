...
pub(super) fn run_command(cmd: &str, env_path: &str, args: &[&str]) -> Result<()> {
...
pub(super) fn backup_list(dir: &Path) -> Result<Vec<DirBackupInfo>> {
    let mut backup_info: Vec<DirBackupInfo> = Vec::new();
                    ...
                    if extension == DEFAULT_ZIP_EXTENSION {
                        ...
                        if let Some(file_name) = path.file_name() {
                            if let Some(file_name) = file_name.to_str() {
                                backup_info.push(DirBackupInfo {
